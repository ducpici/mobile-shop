...
export const ProductCard = ({ id, name, mainImage, price, rating }: ProductCardProps) => {
  return (
    <div className="w-full max-w-sm overflow-hidden rounded-lg bg-white p-2 shadow transition-shadow duration-200 hover:shadow-md">
      <Link href={`/product/${id}`}>
        <div className="relative aspect-square w-full cursor-pointer">
          <Image className="object-cover" src={mainImage} alt={name} fill />
        </div>
      </Link>
